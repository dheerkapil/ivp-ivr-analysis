...
def backfill(days=365):
    ...
    # Stop at 2024-01-01 as data before that is not available
    min_date = datetime(2024, 1, 1)

    while downloaded_days < days and current >= min_date:
    ...
    if downloaded_days < days:
        print(f"⚠️ Only {downloaded_days} trading days found (target: {days}). Reached 2024-01-01 – no older data.")
